...
createLoadHandler = (binding) => function loadHandler () {
  const lazyStatus = this.getAttribute('lazy') // support vue-lazyload
  if (!lazyStatus || lazyStatus === 'loaded') {
    const options = typeof binding.value === 'object' ? binding.value : {}
    collectImage(this,options)
  }
}

function collectImage ($img, itemOptions = {}) {
  ...
  const item = Object.assign({}, itemOptions, { src: originSrc || src, msrc: src, title, w, h });
...
export default function createPreviewDirective (options, PhotoSwipe, PhotoSwipeUI) {
  ...
  return {
    inserted (el, binding) {
      ...
      el.addEventListener('load', createLoadHandler(binding))
