...
const createLoadHandler = (binding) => function loadHandler () {
...
function collectImage ($img, itemOptions = {}) {
  ...
  const item = Object.assign({}, { src: originSrc || src, msrc: src, title, w, h }, itemOptions)
...
export default function createPreviewDirective (options, PhotoSwipe, PhotoSwipeUI) {
  ...
  return {
    inserted (el, binding) {
      ...
      el._loadHandler = createLoadHandler(binding)
